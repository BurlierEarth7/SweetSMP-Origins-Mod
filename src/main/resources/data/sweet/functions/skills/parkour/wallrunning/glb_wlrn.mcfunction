...
execute as @a at @s anchored feet unless block ^-0.5 ^ ^ air if block ~ ~-1 ~ air run tag @s[tag=parkour, tag=!wr] add wr_begin
execute as @a at @s anchored feet unless block ^0.5 ^ ^ air if block ~ ~-1 ~ air run tag @s[tag=parkour, tag=!wr] add wr_begin
...
execute as @a[tag=parkour, tag=wr, tag=can_wallrun, tag=toggle_wallrun] at @s anchored feet if block ^-0.5 ^ ^ air if block ^0.5 ^ ^ air run effect clear @s minecraft:levitation
execute as @a[tag=parkour, tag=wr, tag=can_wallrun, tag=toggle_wallrun] at @s anchored feet if block ^-0.5 ^ ^ air if block ^0.5 ^ ^ air run effect clear @s minecraft:speed
execute as @a[tag=parkour, tag=wr, tag=can_wallrun, tag=toggle_wallrun] at @s anchored feet if block ^-0.5 ^ ^ air if block ^0.5 ^ ^ air run tag @s remove wr
